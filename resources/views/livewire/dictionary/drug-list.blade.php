<div>
    <x-header-navigation class="breadcrumb-form">
        <x-slot name="title">{{ __('dictionaries.drug_list.title') }}</x-slot>
        ...
        <x-slot name="navigation">
            <div class="flex flex-col gap-4" x-data="{ showFilter: false }">
                ...
                {{-- Additional filters --}}
                <div x-cloak x-show="showFilter" x-transition class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div class="form-group group">
                        <input
                            wire:model="innmName"
                            type="text"
                            ...
                            autocomplete="off"
                        />
                        <label for="innmName" class="label peer-valid:text-blue-600 peer-focus:text-blue-600">
                            {{ __('dictionaries.drug_list.inn_name') }}
                        </label>
                    </div>

                    <div class="form-group group">
                        <select wire:model="innmDosageForm" id="innmDosageForm" class="peer input-select w-full">
                            <option value="" selected>{{ __('forms.select') }}</option>
                            @foreach ($this->dictionaries['MEDICATION_FORM'] as $key => $value)
                                <option value="{{ $key }}">{{ $value }}</option>
                            @endforeach
                        </select>
                        <label for="innmDosageForm" class="label peer-valid:text-blue-600 peer-focus:text-blue-600">
                            {{ __('dictionaries.drug_list.dosage_form') }}
                        </label>
                    </div>

                    <div class="form-group group">
                        <select wire:model="medicationCodeAtc" id="medicationCodeAtc" class="peer input-select w-full">
                            <option value="" selected>{{ __('forms.select') }}</option>
                            @foreach (config('ehealth.medications_atc_code') as $code)
                                <option value="{{ $code }}">{{ $code }}</option>
                            @endforeach
                        </select>
                        <label for="medicationCodeAtc" class="label peer-valid:text-blue-600 peer-focus:text-blue-600">
                            {{ __('dictionaries.drug_list.medication_code_atc') }}
                        </label>
                    </div>

                    <div class="form-group group">
                        <select wire:model="mrBlankType" id="mrBlankType" class="peer input-select w-full">
                            <option value="" selected>{{ __('forms.select') }}</option>
                            @foreach ($this->dictionaries['MR_BLANK_TYPES'] as $key => $value)
                                <option value="{{ $key }}">{{ $value }}</option>
                            @endforeach
                        </select>
                        <label for="mrBlankType" class="label peer-valid:text-blue-600 peer-focus:text-blue-600">
    ...
    @if ($drugs->isNotEmpty())
        <section class="shift-content mt-6 max-w-7xl pl-3.5">
                ...
                <fieldset class="fieldset p-6 sm:p-8">
                    ...
                    <div class="space-y-2 text-gray-900 dark:text-gray-100">
                        <p class="font-semibold">
                            {{ __('dictionaries.mr_blank_type') }}: <span>{{ $drug['mr_blank_type'] }}</span>
                        </p>
                        <p>
                            {{ __('dictionaries.drug_list.dosage_form_is_dosed') }}:
                        ...
                        @foreach ($drug['ingredients'] as $ingredient)
                            <p class="font-semibold">
                                {{ __('dictionaries.drug_list.ingredient.name') }}:
                                <span>{{ $ingredient['name'] }}</span>
                            </p>
                            <p class="font-semibold">
                                {{ __('dictionaries.drug_list.ingredient.is_primary') }}:
                                <span>{{ $ingredient['is_primary'] ? __('forms.yes') : __('forms.no') }}</span>
                            </p>
                            <p class="font-semibold">
                                {{ __('dictionaries.drug_list.ingredient.dosage.numerator_value') }}:
                                <span>{{ $ingredient['dosage']['numerator_value'] }}</span>
                            </p>
                            <p class="font-semibold">
                                {{ __('dictionaries.drug_list.ingredient.dosage.numerator_unit') }}:
                                <span>{{ $this->getMedicationUnit($ingredient['dosage']['numerator_unit']) }}</span>
                            </p>
                            <p class="font-semibold">
                                {{ __('dictionaries.drug_list.ingredient.dosage.denumerator_value') }}:
                                <span>{{ $ingredient['dosage']['denumerator_value'] }}</span>
                            </p>
                            <p class="font-semibold">
                                {{ __('dictionaries.drug_list.ingredient.dosage.denumerator_unit') }}:
                        ...
                        @if ($drug['packages'])
                            ...
                            @foreach ($drug['packages'] as $package)
                                <p class="font-semibold">
                                    {{ __('dictionaries.drug_list.package.container_quantity') }}:
                                    <span>{{ $package['container_dosage']['numerator_value'] }}</span>
                                </p>
                                <p>
                                    {{ __('dictionaries.drug_list.package.container_quantity_unit') }}:
                                    <span>{{ $this->getMedicationUnit($package['container_dosage']['numerator_unit']) }}</span>
                                </p>
                                <p>
                                    {{ __('dictionaries.drug_list.package.primary_packages_count') }}:
                                    <span>{{ $package['container_dosage']['denumerator_value'] }}</span>
                                </p>
                                <p>
                                    {{ __('dictionaries.drug_list.package.primary_package_unit') }}:
                                    <span>{{ $this->getMedicationUnit($package['container_dosage']['denumerator_unit']) }}</span>
                                </p>
                                <p>
                                    {{ __('dictionaries.drug_list.package.min_sale_quantity') }}:
