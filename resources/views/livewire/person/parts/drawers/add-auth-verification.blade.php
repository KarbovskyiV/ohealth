...
<div
    ...
    x-data="{
        ...
        startTimer() {
            ...
            this.timerInterval = setInterval(() => {
                if (this.localTimer > 0) this.localTimer--;
            }, 1000);
        },
        resetTimer() {
    ...
    x-init="
        $watch('showAuthDrawer', (value) => {
            if (value) startTimer();
        })
    "
>
    <div class="border-b border-gray-200 bg-white px-6 py-4 dark:border-gray-700 dark:bg-gray-800">
        <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ __('patients.authentication_SMS') }}</h2>
    </div>
    ...
    <div class="overflow-y-auto bg-white p-6 dark:bg-gray-800" style="height: calc(100% - 70px)">
        ...
        <div class="form-row-3 mt-8">
            <div class="form-group group">
                <input
                    ...
                    maxlength="4"
                />
                <label for="authVerificationCode" class="label"> {{ __('forms.confirmation_code_from_SMS') }} </label>
            </div>
            ...
            <button
                ...
                class="button-minor mt-4 mb-8 flex max-w-xs flex-1 items-end gap-4"
            >
                @icon('mail', 'w-4 h-4')
        ...
        <div class="flex gap-3">
            <button type="button" @click="showAuthDrawer = false" class="button-minor">{{ __('forms.back') }}</button>
