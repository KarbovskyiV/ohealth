    ...
    <section>
        <x-header-navigation 
            x-data="{ 
                ...
                focusNext(el) {
                    ...
                    if (!container) return;
                    let elements = Array.from(container.querySelectorAll('input:not([readonly]):not([type=hidden]):not([type=checkbox]), select, button.button-primary')).filter(element => element.offsetWidth > 0 && element.offsetHeight > 0);
        ...
        >
            ...
            <x-slot name="navigation">
                <div class="mb-8 block justify-end sm:flex sm:items-center gap-4">
                    <button class="flex items-center gap-2 text-blue-600 hover:text-blue-800 font-medium text-sm">
                        @icon('plus', 'w-4 h-4')
                ...
                <div class="form-row-3 mb-4">
                    <div class="form-group group">
                        ...
                        />
                        <label for="filterEmployeeId" class="label">
                            {{ __('forms.employee_id') }}
                        </label>
                    </div>
        ...
        <div class="space-y-6 pl-3.5 mb-16">
            @if($hasResults)
                ...
                <div class="mt-2 flex flex-wrap items-center justify-between gap-4 pb-4">
                    <div class="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-500">
                        ...
                        <span class="flex min-w-0 items-center gap-1.5 font-medium text-gray-700 dark:text-gray-300">
                            ...
                            <a href="#" class="truncate hover:underline">{{ $searchForm['phone'] ?: '+380XXXXXXXXX' }}</a>
                        </span>
                    ...
                    <div class="flex items-center space-x-6">
                        <a href="#" class="flex cursor-pointer items-center gap-1.5 text-sm font-medium text-blue-600 hover:text-blue-800">
                            @icon('file-lines', 'w-4 h-4')
                            <span>{{ __('patients.view_record') }}</span>
                        </a>
                        <button type="button" class="flex cursor-pointer items-center gap-1.5 text-sm font-medium text-blue-600 hover:text-blue-800">
                    ...
                    <div class="max-w-7xl">
                        <table class="table-input w-full table-auto">
                            <thead class="thead-input">
                                <tr>
                                    <th scope="col" class="th-input text-left text-xs font-bold tracking-wider text-gray-500 uppercase w-1/6">{{ mb_strtoupper(__('patients.verification_sources.drfo')) }}</th>
                                    <th scope="col" class="th-input text-left text-xs font-bold tracking-wider text-gray-500 uppercase w-1/6">{{ mb_strtoupper(__('patients.verification_sources.dracs_death')) }}</th>
                                    <th scope="col" class="th-input text-left text-xs font-bold tracking-wider text-gray-500 uppercase w-1/6">{{ mb_strtoupper(__('patients.verification_sources.dracs_birth')) }}</th>
                                    <th scope="col" class="th-input text-left text-xs font-bold tracking-wider text-gray-500 uppercase w-1/6">{{ mb_strtoupper(__('patients.verification_sources.dms_passport')) }}</th>
                                    <th scope="col" class="th-input text-left text-xs font-bold tracking-wider text-gray-500 uppercase w-1/6">{{ mb_strtoupper(__('patients.verification_sources.unzr')) }}</th>
                                    <th scope="col" class="th-input text-left text-xs font-bold tracking-wider text-gray-500 uppercase w-1/6">{{ mb_strtoupper(__('patients.verification_sources.nhs')) }}</th>
                                    <th scope="col" class="th-input text-center text-xs font-bold tracking-wider text-gray-500 uppercase w-16">{{ mb_strtoupper(__('forms.action')) }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td class="td-input align-middle whitespace-nowrap"><span class="badge-green px-2 py-0.5 rounded text-xs">{{ __('forms.verified') }}</span></td>
                                    <td class="td-input align-middle whitespace-nowrap"><span class="badge-green px-2 py-0.5 rounded text-xs">{{ __('forms.verified') }}</span></td>
                                    <td class="td-input align-middle whitespace-nowrap"><span class="badge-green px-2 py-0.5 rounded text-xs">{{ __('forms.verified') }}</span></td>
                                    <td class="td-input align-middle whitespace-nowrap"><span class="badge-green px-2 py-0.5 rounded text-xs">{{ __('forms.verified') }}</span></td>
                                    <td class="td-input align-middle whitespace-nowrap"><span class="badge-green px-2 py-0.5 rounded text-xs">{{ __('forms.verified') }}</span></td>
                                    ...
                                    <td class="td-input text-center align-middle">
                                        <div class="relative inline-block" x-data="{ openInteractionDropdown: false }" @click.outside="openInteractionDropdown = false">
                                            <button @click="openInteractionDropdown = ! openInteractionDropdown" class="inline-block cursor-pointer rounded-full p-1.5 transition-colors hover:bg-gray-100 dark:hover:bg-gray-700" title="{{ __('forms.action') }}" type="button">
                                                @icon('edit-user-outline', 'w-6 h-6 text-gray-800 dark:text-gray-200')
                                            </button>

                                            <div x-show="openInteractionDropdown" x-transition x-cloak class="absolute right-0 z-50 mt-2 w-64 rounded-lg border border-gray-200 bg-white py-1 text-left shadow-md dark:border-gray-600 dark:bg-gray-700">
                                                <a
            ...
            @else
                <div class="shift-content max-w-6xl mt-6">
                    <x-nothing-found />
                </div>
            @endif
        </div>
