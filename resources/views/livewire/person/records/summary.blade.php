            ...
            <div class="flex-1 space-y-4">
                @foreach($navItems as $item)
                    ...
                    >
                        ...
                        <div class="shrink-0 text-gray-400 dark:text-gray-500 ml-4">
                            @icon('chevron-down', 'w-4 h-4 transform rotate-90')
                        </div>
                    </button>
                @endforeach
            </div>

            <div class="w-full lg:w-[320px] flex-shrink-0 space-y-1 mt-4 lg:mt-0">
                @foreach($navItems as $item)
                    <button @if($item['action']) wire:click.once="{{ $item['action'] }}" @endif
                            @click="activeSection = '{{ $item['id'] }}'"
                            type="button"
